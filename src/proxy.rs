...
impl Proxy {
        ...
        T: IntoTargetAddr<'t>,
    {
        ...
        return match self.intercept {
            ...
            ProxyScheme::Https { .. } => {
                ...
                let tls_type_opt = get_cached_tls_type(&url);
                let danger_accept_invalid_cert = get_cached_tls_accept_invalid_cert(&url);
                let stream = self
                    .https_connect_rustls_wrap_danger(
                        &url,
                        local,
                        proxy,
                        Some(stream),
                        &target_addr,
                        tls_type_opt.is_some(),
    ...
    async fn https_connect_rustls_wrap_danger<'a>(
        ...
        .await?
        {
            ...
            Err(e) => {
                // NOTE: Maybe it's better to check if the error is related to TLS here. (ProxyError::IoError(e))
